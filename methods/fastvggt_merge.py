...
# ── Module-level merge index cache (for RegimeVGGT's Δ-layer reuse) ──
# Stores (merge_fn, unmerge_fn) closures keyed by caller-supplied cache_key.
# The closures depend only on source/dst/protected indices, so they can be
# safely applied to tokens from later layers within the same Δ-layer window.
_MERGE_CACHE: dict = {}


def clear_merge_cache():
    """Reset the merge cache. Must be called at the start of each scene inference."""
    _MERGE_CACHE.clear()
...
def token_merge_bipartite2d(
    ...
    # Δ-layer index cache: reuse merge/unmerge closures across consecutive layers
    if cache_key is not None and cache_key in _MERGE_CACHE:
    ...
    with torch.no_grad():
        # Protection strategy selection (paper: geometry-aware top-α; fallback: uniform stride)
        if enable_protection:
            if importance_scores is not None:
                # Paper Eq. (3): top-α per-frame importance protection.
                # importance_scores shape: [num_imgs, T_patches], T_patches = w*h.
                # Frame 0 is implicitly protected (all its tokens are marked dst below),
                # so we only select top-α*T from frames 1..num_imgs-1.
                T_patches = w * h
                num_protected_per_frame = max(1, int(T_patches * alpha))
                if (importance_scores.shape[0] != num_imgs
                        or importance_scores.shape[1] != T_patches):
                    raise ValueError(
                        f"importance_scores shape {tuple(importance_scores.shape)} "
                        f"does not match (num_imgs={num_imgs}, T_patches={T_patches})"
                    )
                if num_imgs > 1:
                    imp_rest = importance_scores[1:]  # [num_imgs-1, T_patches]
                    _, topk_local = imp_rest.topk(num_protected_per_frame, dim=1)
                    # local patch idx i in frame s → global idx s*tokens_per_img + 5 + i
                    frame_offsets = (
                        torch.arange(1, num_imgs, device=metric.device)
                        .unsqueeze(-1) * tokens_per_img + 5
                    )  # [num_imgs-1, 1]
                    protected_indices = (
                        (frame_offsets + topk_local).flatten().to(torch.long)
                    )
                else:
                    protected_indices = torch.empty(
                        0, device=metric.device, dtype=torch.long
                    )
                num_protected = protected_indices.shape[0]
            else:
                # Uniform stride fallback (FastVGGT default)
        ...
        dst_global = b_idx.squeeze(0).squeeze(-1)  # [num_dst]
        if enable_protection:
            protected_global = protected_idx.squeeze(0).squeeze(-1)  # [num_protected_actual]
            merged_global_idx = torch.cat(
                [unm_global, dst_global, protected_global], dim=0
            )
        else:
            merged_global_idx = torch.cat([unm_global, dst_global], dim=0)

        # ToMe size vector: count of original tokens each output position
        # represents, ordered as [unm, dst, protected] matching merged_global_idx.
        # Used by attention to add log(size) bias pre-softmax so a dst that
        # represents r originals contributes ∝ r·v_dst (not v_dst alone).
        unm_size = torch.ones(
            unm_idx.shape[1], device=metric.device, dtype=torch.float32
        )
        dst_count = torch.zeros(num_dst, device=metric.device, dtype=torch.float32)
        if r_actual > 0:
            src_to_dst_idx = dst_idx.squeeze(0).squeeze(-1)
            dst_count.scatter_add_(
                0,
                src_to_dst_idx,
                torch.ones_like(src_to_dst_idx, dtype=torch.float32),
            )
        dst_size = dst_count + 1.0  # +1: dst itself was also an original token
        if enable_protection:
            protected_size = torch.ones(
                num_protected_actual, device=metric.device, dtype=torch.float32
            )
            merge_size = torch.cat([unm_size, dst_size, protected_size], dim=0)
        else:
            merge_size = torch.cat([unm_size, dst_size], dim=0)
    ...
    unmerge.merged_global_idx = merged_global_idx
    # ToMe size vector for size-aware attention bias (training-free).
    merge.size = merge_size
    unmerge.size = merge_size

    # Cache closures for Δ-layer reuse (RegimeVGGT cache_interval)
    if cache_key is not None:
        _MERGE_CACHE[cache_key] = (merge, unmerge)
