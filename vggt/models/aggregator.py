...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> Tuple[List[torch.Tensor], int]:
        ...
        skip_layers = getattr(self, '_skip_layers', set())
        skip_global_layers = getattr(self, '_skip_global_layers', set())
        ...
        for block_num in range(self.aa_block_num):
            ...
            # Layer skip: bypass frame+global attention, record DPT output if needed
            if block_num in skip_layers:
                if need_intermediates:
                    inter = tokens.view(B, S, P, C) if tokens.dim() == 3 else tokens
                    if inter.dim() == 3:
                        inter = inter.view(B, S, P, C)
                    concat_inter = torch.cat([inter, inter], dim=-1)
            ...
            for attn_type in self.aa_order:
                if attn_type == "frame":
                    ...
                    # Skip global attention for specified layers (shallow skip)
                    if block_num in skip_global_layers:
                        global_intermediates = frame_intermediates if need_intermediates else None
                        continue
